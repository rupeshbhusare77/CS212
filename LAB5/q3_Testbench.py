...
sim_time = []
channel_utilization = []
fraction_of_retransmited_packets = []
...
for i in range(1,10):
    ...
    channel_for_data  = UnreliableChannel(env=env,name="DATA_CHANNEL",Pc=0.2,Pl=i/10,propagation_delay=2, transmission_rate=1000)
    channel_for_ack	  = UnreliableChannel(env=env,name="ACK_CHANNEL", Pc=0.2,Pl=i/10,propagation_delay=2, transmission_rate=1000)
    ...
    TOTAL_SIMULATION_TIME=2000000  # <==== Total simulation time. Increase it as you like.
    t=0
    ...
    sim_time.append(t)
    channel_utilization.append( round((channel_for_data.channel_utilization_time / t * 100.0),2))
    fraction_of_retransmited_packets.append(round((rdt_sender.num_retransmissions / rdt_sender.total_packets_sent * 100.0),3))
